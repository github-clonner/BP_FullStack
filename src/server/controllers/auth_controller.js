...
exports.resetPassword = (req, res) => {
  User.findOne({ resetPasswordToken: req.body.token, resetPasswordExpires: { $gt: Date.now() } }, (err, resetUser) => {
    ...
    resetUser.save((resetErr) => {
      if (resetErr) { return res.send(422).json({ error: 'Error saving user' }) }
      ...
      mailgun.sendEmail(resetUser.email, message)
        .then((response) => {
          return res.status(200).json({ success: 'Password successfully changed. Please login with your new password.' })
        })
        .catch((err) => {
          return res.status(422).json({error: 'Oops something went wrong dispatching to your email.  Please contact support' })
        })
    })
  })
}
...
exports.forgotPassword = (req, res) => {
  ...
  User.findOne({ email }, (err, existingUser) => {
    ...
    crypto.randomBytes(48, (cryptoErr, buffer) => {
      const resetToken = buffer.toString('hex')
      if (cryptoErr) { return res.send(422).json({ error: 'Oops! Something went wrong generating a token! Please contact support' }) }

      existingUser.resetPasswordToken = resetToken
      existingUser.resetPasswordExpires = Date.now() + 3600000

      existingUser.save((UserSaveErr) => {
        if (UserSaveErr) { return res.send(422).json({ error: 'Error saving your user token. Please contact support' }) }
        ...
        mailgun.sendEmail(existingUser.email, message)
          .then((response) => {
            return res.status(200).json({ success: 'Please check your email for the link to reset your password.' })
          })
          .catch((err) => {
            return res.status(422).json({error: 'Oops something went wrong dispatching to your email.  Please contact support' })
          })
  ...
  User.findOne({ email: email }, function (err, existingUser) {
    ...
    user.save(function (saveErr, user) {
      ...
      mailgun.sendEmail(user.email, message)
        .then((response) => {
          return res.status(200).json({ success: 'Thanks for signing up!  Email sent!' })
        })
        .catch((err) => {
          return res.status(422).json({ info: 'Signed up! But there was an issue with your email.  Please double check your inbox' })
        })
